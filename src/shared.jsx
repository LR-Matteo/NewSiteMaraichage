...
const Placeholder = ({ label, ratio = '4/3', tone = 'creme', style = {} }) => {
  const bgs = { creme: ORSTokens.sable, vert: ORSTokens.vertSoft, sable: ORSTokens.cremeDark, dark: ORSTokens.vertDark };
  const textColors = { creme: ORSTokens.charbonSoft, vert: ORSTokens.creme, sable: ORSTokens.charbonSoft, dark: ORSTokens.creme };
  return (
    <div style={{
      aspectRatio: ratio,
      background: `repeating-linear-gradient(135deg, transparent 0 14px, rgba(0,0,0,0.025) 14px 15px), ${bgs[tone]}`,
      position: 'relative', overflow: 'hidden', ...style,
    }}>
      <div style={{ position: 'absolute', top: 12, left: 14, fontFamily: 'ui-monospace, monospace', fontSize: 10, letterSpacing: 0.8, textTransform: 'uppercase', color: textColors[tone], opacity: 0.7 }}>img</div>
      <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: ORSTokens.sans, fontSize: 13, color: textColors[tone], opacity: 0.75, textAlign: 'center', padding: 20 }}>{label}</div>
    </div>
  );
};
...
const FadeIn = ({ children, delay = 0, distance = 30, style = {}, ...props }) => {
  ...
  return (
    <div ref={ref} style={{ opacity: on ? 1 : 0, transform: on ? 'translateY(0)' : `translateY(${distance}px)`, transition: `opacity .7s ease ${delay}ms, transform .7s ease ${delay}ms`, ...style }} {...props}>
      {children}
    </div>
  );
};
...
export { ORSTokens, ORSPalettes, setORSPalette, Picto, Placeholder, FadeIn, MapContact };
