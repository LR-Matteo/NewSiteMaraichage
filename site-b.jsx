// Variante B — Éditorial audacieux
// Hero full-bleed, composition asymétrique, grande typographie dramatique
...
function ContactFormBlock({ t, compact = false }) {
  ...
  async function submit(e) {
    ...
    setStatus('sending');
    try {
      await window.emailjs.send('service_lfgrmkv', 'template_1er3wnb', {
        name: form.name.trim(),
...
// Données de démo — utilisées si Supabase ne renvoie rien
const PRODUITS_B = [
              ...
              {[
                ...
                <div key={gi} style={{ padding: '14px 20px', borderBottom: gi < arr.length - 1 ? `1px solid ${t.bord}` : 'none' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 10 }}>
                    <Picto name={g.icone} size={11} color={g.couleur} stroke={1.8} />
                    <span style={{ fontSize: 10, fontWeight: 700, letterSpacing: 1.5, textTransform: 'uppercase', color: g.couleur }}>{g.jour}</span>
                  </div>
                  {g.arrets.map((a, ai) => (
                    <div key={ai} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', gap: 8, marginBottom: ai < g.arrets.length - 1 ? 8 : 0 }}>
